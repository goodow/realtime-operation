...
public class MapOp implements Operation<MapTarget>, MapTarget {
  ...
  public Pair<MapOp, MapOp> transformWith(Operation<?> clientOp) {
    ...
    for (int i = 0, len = clientKeys.length(); i < len; i++) {
      ...
      if (!JsonUtility.jsonEqual(serverOldValue, clientOldValue)) {
      ...
      if (JsonUtility.jsonEqual(serverNewValue, clientNewValue)) {
        continue;
      }
  ...
  public MapOp update(String key, JsonValue oldValue, JsonValue newValue) {
    ...
    if (JsonUtility.jsonEqual(oldValue, newValue)) {
      return this;
    }
    ...
    if (!JsonUtility.jsonEqual(components.get(key).second, oldValue)) {
    ...
    if (JsonUtility.jsonEqual(components.get(key).first, newValue)) {
