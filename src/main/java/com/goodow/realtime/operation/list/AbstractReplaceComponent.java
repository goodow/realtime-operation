...
public abstract class AbstractReplaceComponent<T> extends AbstractListComponent<T> {
    ...
    switch (op.type) {
      ...
      case AbstractDeleteComponent.TYPE:
        if (op.startIndex >= endIndex0) {
          ...
          // ....{...}....
          // ...[..]...]
          return endIndex1 < endIndex0 ? create(op.startIndex, canUndo() ?
              getHelper().subset(oldValues, endIndex1 - startIndex, endIndex0 - endIndex1) : null,
              getHelper().subset(values, endIndex1 - startIndex, endIndex0 - endIndex1)) : null;
        } else {
          // ....{...}....
          // .....[.]..]
          return endIndex1 < endIndex0 ? create(startIndex, canUndo() ? getHelper().replaceWith
              (oldValues, op.startIndex - startIndex, op.length, null) : null, getHelper()
              .replaceWith(values, op.startIndex - startIndex, op.length, null)) : create(
              startIndex, canUndo() ? getHelper().subset(oldValues, 0, op.startIndex - startIndex)
              : null, getHelper().subset(values, 0, op.startIndex - startIndex));
        }
      case AbstractReplaceComponent.TYPE:
        if (endIndex1 <= startIndex || op.startIndex >= endIndex0
          ...
          if (!applied) {
            T transformedOldValues =
                endIndex1 < endIndex0 ? getHelper().subset(op.values, startIndex - op.startIndex,
                  endIndex1 - startIndex, oldValues, endIndex1 - startIndex, endIndex0 - endIndex1)
                : getHelper().subset(op.values, startIndex - op.startIndex, length);
            return create(startIndex, transformedOldValues, values);
          } else {
            return endIndex1 < endIndex0 ? create(endIndex1, canUndo() ? getHelper().subset
                (oldValues, endIndex1 - startIndex, endIndex0 - endIndex1) : null,
                 getHelper().subset(values, endIndex1 - startIndex, endIndex0 - endIndex1)) : null;
          }
        } else {
          // ....{...}....
          // .....{.}..}
          if (!applied) {
            T transformedOldValues = endIndex1 < endIndex0 ?
                getHelper().replaceWith(oldValues, op.startIndex - startIndex, op.length, op.values)
                : getHelper().subset(oldValues, 0, op.startIndex - startIndex, op.values, 0,
                    endIndex0 - op.startIndex);
            return create(startIndex, transformedOldValues, values);
          } else if (endIndex1 >= endIndex0) {
            // ....{...}....
            // .....{....}
            return create(startIndex,
                   canUndo() ? getHelper().subset(oldValues, 0, op.startIndex - startIndex) : null,
                   getHelper().subset(values, 0, op.startIndex - startIndex));
          } else {
            return super.transform(other, applied);
          }
        }
  ...
  @Override
  public OperationComponent<ListTarget<T>>[] transformComponent(
      OperationComponent<ListTarget<T>> other, boolean applied) {
    AbstractListComponent<T> op = (AbstractListComponent<T>) other;
    int endIndex0 = startIndex + length;
    int endIndex1 = op.startIndex + op.length;
    switch (op.type) {
      case AbstractInsertComponent.TYPE:
        if (op.startIndex > startIndex && op.startIndex < endIndex0) {
          ...
          return asArray(create(startIndex, canUndo() ?
                 getHelper().subset(oldValues, 0, len0) : null, getHelper().subset(values, 0, len0))
          , create(endIndex1, canUndo() ? getHelper().subset(oldValues, len0, length - len0) : null,
        ...
        if (applied && op.startIndex > startIndex && endIndex1 < endIndex0) {
          AbstractReplaceComponent<T> op1 =
              create(startIndex,
                    canUndo() ? getHelper().subset(oldValues, 0, op.startIndex - startIndex) : null,
                    getHelper().subset(values, 0, op.startIndex - startIndex));
          return asArray(op1, create(endIndex1, canUndo() ?
                getHelper().subset(oldValues, endIndex1 - startIndex, endIndex0 - endIndex1) : null,
